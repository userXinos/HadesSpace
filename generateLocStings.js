...
function loadStrings(lang) {
  const result = {};
  const strings = fs.readFileSync(`${path}loc_strings_${lang}.txt`, 'utf8').split('\n');
...
function addStrings(obj, lang) {
  const jsonStrings = require(`${path}modification/addStringsFile.js`, 'utf8').default;
  const defaultLang = require(`${path}modification/addStringsFile.js`, 'utf8').default.en;
  obj = combineObjects(obj, jsonStrings[lang]);
  const addStringsToJson = {
  ...
  const result = {};
  const keys = Object.keys(addStringsToJson);

  for (const key of keys) {
    const items = addStringsToJson[key];
    for (const i of items) {
      const startVal = obj[key] || defaultLang[key];
...
function isTrash(key) {
  const trash = JSON.parse(fs.readFileSync(`${path}modification/trashStrings.json`, 'utf8'));
...
function fixWorlds(rawStr) {
  ...
  if (firstLetterRaw.toUpperCase() == firstLetterRaw) { // первая буква была заглавной ?
    const result = lower.replace(firstLetter, firstLetter.toUpperCase());
