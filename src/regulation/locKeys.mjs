// noinspection SpellCheckingInspection

export const gameSlostLocKeys = {
    Trade: 'TID_MODULE_BLUEPRINTS_LIST_CATEGORY_TRADE',
    Mining: 'TID_MODULE_BLUEPRINTS_LIST_CATEGORY_MINING',
    Weapon: 'TID_MODULE_BLUEPRINTS_LIST_CATEGORY_WEAPONS',
    Shield: 'TID_MODULE_BLUEPRINTS_LIST_CATEGORY_SHIELDS',
    Combat: 'TID_MODULE_BLUEPRINTS_LIST_CATEGORY_SUPPORT',
    Support: 'TID_MODULE_BLUEPRINTS_LIST_CATEGORY_SUPPORT',
    Drone: 'TID_MODULE_BLUEPRINTS_LIST_CATEGORY_DRONE',
}

export const gameCharsLocKeys = {
    BlueStar_Fragments: 'TID_BS_FRAGMENTS_REWARD_TITLE',
    RushYSHydroDc: 'TID_MODULE_RUSH_HYDRO_REDUCT',
    RushRSHydroDc: 'TID_MODULE_RUSH_HYDRO_REDUCT',
    RushRSSpeedIncr: 'TID_MODULE_RUSH_SPEED_INCR',
    RushYSSpeedIncr: 'TID_MODULE_RUSH_SPEED_INCR',
    RushWSSpeedIncrPR: 'TID_MODULE_RUSH_SPEED_INCR',
    DroneHydro: 'TID_MODULE_DESCR_ACTIVATION_COST',
    ActivationHydro: 'TID_MODULE_DESCR_ACTIVATION_COST',
    WSDispatchAOEAmt: 'TID_DESTROYER_AREADAMAGE',
    HydroDeliveryBonus: 'TID_MODULE_BONUS_HYDRO',
    OnBoardHydroMax: 'TID_MODULE_ONBOARD_HYDRO_USE',
    RelayBonusPct: 'TID_RELAY_BONUS_PCT',
    SHHydroPerDay: 'TID_HARVESTER_HYDRO_PER_DAY_INFO',
    ImpulseDPS: 'TID_MODULE_IMPULSE_DAMAGE',
    MassMiningTotalAsteroids: 'TID_MODULE_MASS_MINING_MAX_ASTEROIDS',
    MassMiningRatePct: 'TID_MODULE_MASS_MINING_REMOTE_RATE',
    WSDispatchAOERange: 'TID_DESTROYER_AREADAMAGERANGE',
    DispatchActivTicks: 'TID_DISPATCH_PREP_TIME_FIXED',
    AdditionalRangesPerLink: 'TID_MODULE_SUPPORT_ADDITIONAL_RANGE',
    LinkDPSBoost: 'TID_MODULE_SUPPORT_ADDITIONAL_DPS',
    LeapAOEAmt: 'TID_DESTROYER_AREADAMAGE',
    DamageAmplifyFactor: 'TID_MODULE_DAMAGE_AMPLIFY_STAT',
    DamageReduceFactor: 'TID_MODULE_DAMAGE_REDUCTION_STAT',
    MirrorDamageAsAOE: 'TID_MODULE_MIRROR_DAMAGE_STAT',
    DroneSquadBattery: 'TID_MODULE_BATTERY',
    ArtifactBoostPct: 'TID_MODULE_ARTIFACT_BOOST_BONUS',
    MaxRelicSpeedIncrease: 'TID_MODULE_RUSH_MAX_WS',
    ImpulseRange: 'TID_DESTROYER_AREADAMAGERANGE',
    ImpulseSpeedup: 'TID_MODULE_IMPULSE_SPEEDUP',
    SalvageAmount: 'TID_MODULE_SALVAGE_HULL_DESCR',
    SolitudeDPSBoost: 'TID_MODULE_STAT_DAMAGE_INCR',
    FortifyDamageAmpFactor: 'TID_MODULE_STAT_DAMAGE_INCR',
    AddDPSPerTarget: 'TID_MODULE_DESCR_DPADT',
    RelicLoadTicks: 'TID_MODULE_RELIC_DRONE_SPEED',
    ActivationType: 'TID_MODULE_TYPE',
    StarSectorDPS: 'STAR_SECTOR_DPS',
    RSLevel: 'TID_XBL_STAT_4',
    PriceInCrystals: 'PRICE_IN_CRYSTALS',
    Influence: 'TID_CORP_SEARCH_INFLUENCE_RANGE',
    AllowMoveInSameSectorOnly: 'TID_HUD_MUST_MOVE_TO_SAME_SECTOR',
    RequiresStarSector: 'TID_HUD_PLACE_LOCATION_INVALID_NO_SUN',
    CanCancelBuild: 'TID_OBJ_ACTION_CANCEL_BUILD',
    RequiresEmptySector: 'TID_HUD_PLACE_LOCATION_INVALID_MUST_BE_EMPTY_SECTOR',
    DisableTime: 'TID_MODULE_LEAP_DISABLE_STAT',
    modules: 'TID_OBJECT_ACTION_SHIP_UPGRADE',
    MiningPeriod: 'TID_MODULE_MINING_SPEED_STAT',
    SecurityRating: 'TID_SECURITY_RATING',
    LaserTurret_Laser: 'TID_MODULE_LASER_TURRET',
    AdditionalWaypoint: 'TID_OBJECT_ACTION_ADD_WAYPOINT',
    IsStealth: 'TID_MODULE_STEALTH',
    MaxHP: 'TID_FLEET_HULL_STRENGTH_MAX',
    MaxShield: 'TID_MODULE_SHIELD_STRENGTH_STAT',
    ShieldRegenPerMinute: 'TID_CERBERUS_STATION_SHIELD_REGEN_LABEL',
    ShieldRegenTimeAfterDamage: 'TID_CERBERUS_STATION_SHIELD_REGEN_DELAY',
    SpawnFleetIntervalSeconds: 'TID_CERBERUS_STATION_SPAWNING_IN',
    AttackRange: 'TID_MODULE_ATTACK_RANGE_DESCR',
    CroidAmt: 'TID_CR_ASTEROID_FIELD',
    EMPResist: 'TID_EMP_RESIST',
    TargetSwitchTicks: 'TID_MODULE_DESCR_TARGET_SWITCH_DELAY',
    ProximityTriggerSec: 'TID_MODULE_TRIGGER_TIME',
    TimeSlowdownFactor: 'TID_STAR_TIME_SLOWDOWN',
    PreparationTimeHours: 'TID_WS_PREPARATION_PERIOD',
    RedStar: 'TID_RED_STAR',
    YellowStar: 'TID_YELLOW_STAR',
    WhiteStar: 'TID_WHITE_STAR',
    BlueStar: 'TID_BLUE_STAR',
    alliancelevels: 'TID_CORP_LEVELS_LABEL',
    MaxMembers: 'TID_CORP_MAX_MEMBERS',
    PassiveIncomeModifier: 'TID_CORP_PASSIVE_INCOME_BONUS',
    BaseArtifactYieldBonus: 'TID_ARTIFACT_BONUS_CORP_LABEL',
    MaxArtifactYieldBonus: 'TID_CORP_ARTIFACT_YIELD_BONUS',
    MaxFlagshipLevel: 'TID_CORP_MAX_FLAGSHIP_LEVEL',
    achievements: 'TID_PLAYER_INFO_DLG_TAB_ACHIEVEMENTS',
    UnlockAmount: 'TID_TRADE_DLG_DESTINATION_COLUMN',
    PCReward: 'TID_HUD_CRYSTALS',
    spacebuildings: 'TID_PRODUCTION_DLG_STATIONS',
    playergoals: 'TID_EMPIRE_OBJECTIVES',
    EMP: 'TID_MODULE_EMP',
    Fortify: 'TID_MODULE_FORTIFY',
    Impulse: 'TID_MODULE_IMPULSE',
    Destiny: 'TID_MODULE_DESTINY',
    Vengeance: 'TID_MODULE_VENGEANCE',
    DeltaRocket: 'TID_MODULE_DELTAROCKET',
    Barrier: 'TID_MODULE_BARRIER',
    Bond: 'TID_MODULE_BOND',
    OmegaRocket: 'TID_MODULE_OMEGAROCKET',
    CRReward: 'TID_HUD_CREDITS',
    FuelReward: 'TID_HUD_HYDROGEN',
    ShipyardCapacity: 'TID_ACHIEVEMENT_FLEETSIZE',
    SectorUnlockCost: 'TID_SECTOR_UNLOCK_PRICE',
    MoveHydrogenCostPerSector: 'TID_OBJECT_ACTION_MOVE',
    CancelBuildRefundPct: 'TID_OBJ_ACTION_CANCEL_BUILD',
    RegularInfuenceRange: 'TID_REDSCANNER_OPTIMAL_INFLUENCE',
    MaxBattleshipsPerPlayer: 'TID_REDSCANNER_MAX_BATTLESHIPS',
    HydrogenSearchCost: 'TID_SECTOR_UNLOCK_PRICE',
    DestroyersToSpawn: 'TID_SHIP_CERB_DESTROYER',
    InterceptorsToSpawn: 'TID_SHIP_CERB_INTERCEPTOR',
    TimeToLoad: 'TID_STATUS_LOADING_ARTIFACT',
    Lifetime: 'TID_MISSIONS_DLG_RED_STAR_SUPERNOVA_TIMER',
    CreditIncomeModifier: 'TID_PLANET_UPG_CREDIT_YIELD',
    FuelIncomeModifier: 'TID_PLANET_UPG_HYDROGEN_YIELD',
    CreditStorageModifier: 'TID_PLANET_UPG_CREDIT_STORE',
    FuelStorageModifier: 'TID_PLANET_UPG_HYDROGEN_STORE',
    TID: 'TID_LEADERBOARDS_DIALOG_COLUMN_NAME',
    TID2: 'TID_LEADERBOARDS_DIALOG_COLUMN_NAME',
    TID_Description: 'TID_CORP_DESCR_LABEL',
    Name: 'TID_CORP_NAME_LABEL',
    Name2: 'TID_CORP_NAME_LABEL',
    CreditStorage: 'TID_PLANET_UPG_CREDIT_STORE',
    FuelStorage: 'TID_PLANET_UPG_HYDROGEN_STORE',
    CreditsPerHour: 'TID_PLANET_UPG_CREDIT_YIELD',
    FuelPerHour: 'TID_PLANET_UPG_HYDROGEN_YIELD',
    SectorUnlockTime: 'TID_SECTOR_UNLOCK_TIME',
    AsteroidsMax: 'TID_MODULE_GENESIS_MAX_ASTEROIDS',
    BaseType: 'TID_CERB_STATION_HOME2',
    BSAnomaly: 'TID_BLUE_STAR_ANOMALY',
    BuildCost: 'TID_STAT_SHIP_DESIGN_BUILD_COST',
    JobCapacity: 'TID_TRANSPORT_CAPACITY',
    FuelUsePer5000Distance: 'TID_FLEET_HYDRO_USAGE',
    HealRate: 'TID_FLEET_HEAL_RATE',
    OnDeathHealAmount: 'TID_MOD_REP_DRONE_REP_EXPIRE',
    OnDeathHealAmount_Combat: 'TID_MOD_REP_DRONE_REP_COMBAT',
    Battery: 'TID_MODULE_BATTERY',
    Recoil: 'TID_MODULE_RECOIL',
    Immolation: 'TID_MODULE_IMMOLATION',
    Stealth: 'TID_MODULE_STEALTH',
    EMPRocket: 'TID_MODULE_EMP_ROCKET',
    NumSectorsToMine: 'TID_MINER_AUTO_SECTORS',
    DualLaser: 'TID_MODULE_DUAL_LASER',
    AutoActivateHealth: 'TID_MODULE_HULL_THRESHOLD_STAT',
    InitialModule: 'TID_INSTALLED_MODULES_AREA_TITLE',
    PassiveShield: 'TID_MODULE_PASSIVE_SHIELD',
    InfluencePoints: 'TID_PLAYER_INFO_INFLUENCE_TITLE',
    ActivationHydroOnBoard: 'TID_REQUIRED_HYDROGEN',
    TimeToFullyRegen: 'TID_MODULE_SHIELD_FULL_REGEN_TIME',
    ShieldRegenDelay: 'TID_MODULE_SHIELD_REGEN_DELAY',
    ExtraTradeSlots: 'TID_SHIP_UPGRADE_EFFECT_TRADE_SLOTS',
    FuelUseIncrease: 'TID_SHIP_UPGRADE_EFFECT_FUEL_INCREASE',
    WaypointShipmentRewardBonus: 'TID_SHIP_UPGRADE_EFFECT_BONUS_REWARD',
    ActivationDelay: 'TID_MODULE_COOLDOWN_STAT',
    EffectDurationx10: 'TID_MODULE_EFFECT_DURATION_DESCR',
    MiningSpeedModifierPct: 'TID_MODULE_MINING_SPEED_STAT',
    EffectRadius: 'TID_MODULE_EFFECT_RANGE_DESCR',
    RepairHullPointsPerSecond: 'TID_MODULE_REPAIR_STAT',
    ShieldStrength: 'TID_FLEET_SHIELD_STRENGTH',
    ActivationPrep: 'TID_MODULE_ACTIVATION_PREP_TIME',
    TradeBurstShipmentsStart: 'TID_MODULE_TRADEBURST_THRESHOLD',
    TradeBurstShipmentBonus: 'TID_SHIP_UPGRADE_EFFECT_BONUS_REWARD',
    TeleportShipments: 'TID_MODULE_SHIPMENTBEAM_MAX_SHIPMENTS',
    Speed: 'TID_FLEET_SPEED',
    HP: 'TID_FLEET_HULL_STRENGTH',
    Damage: 'TID_MODULE_PROJECTILE_DAMAGE',
    DamageRange: 'TID_MODULE_PROJECTILE_DAMAGERANGE',
    IncreaseSectorHydroPct: 'TID_MODULE_ENRICH_INCREASE',
    MaxNewAsteroids: 'TID_MODULE_GENESIS_MAX_ASTEROIDS',
    MaxNewHydro: 'TID_MODULE_GENESIS_MAX_HYDRO',
    SpawnLifetime: 'TID_DRONE_LIFETIME',
    DPS: 'TID_FLEET_DPM',
    DPH: 'TID_MODULE_DESCR_DPS_PER_HR',
    DamageRangeWhenNeutralized: 'TID_MODULE_SECONDARY_RANGE',
    HydrogenCapacity: 'TID_HYDRO_CAPACITY',
    AOEDamage: 'TID_DESTROYER_AREADAMAGE',
    DamageWhenNeutralized: 'TID_MODULE_SECONDARY_DAMAGE',
    APTPIOTTP: 'TID_DISPATCH_PREP_TIME',
    HydroUploadPct: 'TID_MODULE_HYDRO_UPLOAD_PERCENT',
    MaxTargets: 'TID_MODULE_MAX_TARGETS',
    FlagshipDartBarrage: 'TID_MODULE_FLAGSHIP_SHIELD',
    FlagshipWeaponModule: 'TID_MODULE_FLAGSHIP_WEAPON',
    FlagshipAreaShield: 'TID_MODULE_FLAGSHIP_WEAPON',
    FlagshipShieldModule: 'TID_MODULE_FLAGSHIP_SHIELD',
    TurretSetupTime: 'TID_TURRET_SETUP_TIME',
    DroneDeliveryBonus: 'TID_MODULE_DRONE_DELIVERY_BONUS',
    RSLESec: 'TID_MODULE_RED_STAR_EXTENSION_STAT',
    BlastDroneDamageAmount: 'TID_MODULE_IMMOLATION_DAMAGE_AMT',
    MotionShieldPct: 'TID_MOTION_SHIELD_PCT',
    MovementSlowdown: 'TID_MODULE_SPEED_SLOWDOWN_STAT',
    LeapAOERange: 'TID_DESTROYER_AREADAMAGERANGE',
    ColonizeCost: 'TID_OBJECT_ACTION_COLONIZE',
    MaxHealth: 'TID_OBJECT_ACTION_COLONIZE',
    ImmolationDamage: 'TID_MODULE_IMMOLATION_DAMAGE_AMT',
    Crunch: 'TID_MODULE_CRUNCH',
    MaxShipments: 'TID_MODULE_SHIPMENTBEAM_MAX_SHIPMENTS',
    ShipmentOverride: 'TID_MODULE_SHIPMENTBEAM_MAX_SHIPMENTS',
    drone: 'TID_MODULE_BLUEPRINTS_LIST_CATEGORY_DRONE',
    XPRequired: 'TID_LEADERBOARD_SHIP_XP',
    RSLevelRequired: 'TID_XBL_STAT_4',
    RSLevelReq: 'TID_XBL_STAT_4',
    BLSDailyCRReward: 'TID_STAT_SHIP_MAX_BLS_CREDIT_INCOME',
    FlagshipStealth: 'TID_MODULE_STEALTH',
    FlagshipDroneSwarm: 'TID_MODULE_FLAGSHIP_DRONES',
    FlagshipRocketCluster: 'TID_MODULE_ROCKET_CLUSTER',
    DroneRocketLauncher: 'TID_MODULE_PLAYER_ROCKET_LAUNCHER',
    ChainRayForDrone: 'TID_MODULE_CHAIN_RAY',
    HydroCostPerShipment: 'TID_RELAY_COST_PER_SHIPMENT',
    PlanetDamagePerSecFP: 'TID_STATUS_DESTROYING_PLANET',
    PeriodSpawnTicks: 'TID_SPAWNER_NUM_SPAWNED',
    PeriodSpawnCount: 'TID_SPAWNER_NUM_SPAWNED',
    ReqUniqueCargo: 'TID_MODULE_CANT_ACTIVATE_NEED_CARGO_ONBOARD',
    OnDeathHealRange: 'TID_MOD_REP_DRONE_RANGE',
    ImpulseDuration: 'TID_MODULE_IMPULSE_DURATION',
    MirrorShieldTickPeriod: 'TID_MODULE_ACTIVATION_PREP_TIME',
    UsesTillCooldown: 'TID_MODULE_USES_TILL_COOLDOWN',
    BlastDroneDamageRange: 'TID_DESTROYER_AREADAMAGERANGE',
    AttackPhaseMaxSpeed: 'TID_FLEET_SPEED',
    YSCommonVariations: 'TID_CUSTOMIZE_OBJECT',
    CenterPlacement: 'TID_BUILDING_CAN_ONLY_MOVE_TO_NEW_SECTOR',
    RequiresSectorDistance: 'TID_HUD_PLACE_LOCATION_INVALID_SECTOR_DISTANCE',
    DRSArtifactBonus: 'TID_MODULE_ARTIFACT_BOOST',
    FleetDepartTimeInSeconds: 'TID_STATUS_DEPARTING',
    DNDiff: 'TID_DN_FIRST_OBJECTIVE_GOAL_TITLE',
    RampDPS: 'TID_MODULE_DESCR_STAGE_DPS',
    DPSRampTimes: 'TID_MODULE_DESCR_STAGE_TIME',
    JumpShips: 'TID_RS_DLG_JUMP_SHIPS_BTN',
    JumpCostTransport: 'TID_SHIP_TRANSPORT',
    JumpCostMiner: 'TID_SHIP_MINER',
    JumpCostBattleship: 'TID_SHIP_BATTLESHIP',
    Sentinel: 'TID_SHIP_CERB_FIGHTER',
    Guardian: 'TID_SHIP_CERB_COLOSSUS',
    Colossu: 'TID_SHIP_CERB_SECTOR_GUARDIAN',
    TimeToResearch: 'TID_OBJ_RESEARCH_ARTIFACTS',
    ShipmentsCRValuePerDay: 'TID_PLANET_UPG_SHIPMENT_YIELD',
    TINDMG: 'TID_MODULE_STAT_TELEPORT_DAMAGE',
};


export default {
    // by i18n/locale
    UnlockBlueprints: 'UNLOCK_BLUEPRINTS',
    UnlockPrice: 'UPGRADE_COST',
    UnlockTime: 'UPGRADE_TIME',
    TeleportBreaksBond: 'TELEPORT_BREAKS_BOND',
    NewModuleSlots: 'NEW_MODULE_SLOTS',
    projectile: 'PROJECTILE',
    MaxPerStarSystem: 'MAX_PER_STAR_SYSTEM',
    MaxPer: 'MAX_PER_STAR_SYSTEM',
    PreventUseOnWsJumpgate: 'PREVENT_USE_ON_WS_JUMPGATE',
    SectorEnrichCooldownSeconds: 'SECTOR_ENRICH_COOLDOWN_SECONDS',
    SectorGenesisCooldownSeconds: 'SECTOR_GENESIS_COOLDOWN_SECONDS',
    BarrageMaxAdditionalEnemies: 'BARRAGE_MAX_ADDITIONAL_ENEMIES',
    StopCountdownOnDisable: 'STOP_COUNTDOWN_ON_DISABLE',
    IsAreaShield: 'AREA_SHIELD',
    BSOnly: 'BS_ONLY',
    UnityMaxDamageIncreasePct: 'UNITY_MAX_DAMAGE_INCREASE_PCT',
    DisableActivationDuringPrep: 'DISABLE_ACTIVATION_DURING_PREP',
    BondOverridesBarrier: 'BOND_OVERRIDES_BARRIER',
    IgnoreBarrier: 'IGNORE_BARRIER',
    LeapBreaksBond: 'LEAP_BREAKS_BOND',
    BondBreaksBond: 'BOND_BREAKS_BOND',
    DeactivateOnJump: 'DEACTIVATE_ON_JUMP',
    HydrogenPerDay: 'HYDROGEN_PER_DAY',
    CerbGroup: 'CERBERUS',
    Cost: 'COST',
    XPAward: 'XP_AWARD',
    TimeToUpgrade: 'TIME_TO_UPGRADE',
    MaxUpgradeLevel: 'MAX_UPGRADE_LEVEL',
    CreditShipmentModifier: 'CREDIT_SHIPMENT_MODIFIER',
    PlanetTypes: 'PLANET_TYPES',
    Model: 'MODEL',
    DesignUpgradeCost: 'UPGRADE_COST',
    DesignUpgradeTime: 'UPGRADE_TIME',
    ApplyAOEDamageOnDestroy: 'AOE_DAMAGE_ON_DESTROY',
    OnDestroySpawnCount: 'ON_DESTROY_SPAWN_COUNT',
    ShipToSpawn: 'SHIP_TO_SPAWN',
    OnDestroySpawn: 'SHIP_TO_SPAWN',
    MaxShips: 'MAX_SHIPS',
    AwardXP: 'XP_AWARD',
    RadialMoveRadius: 'MOVE_RADIUS',
    MaxOnOwnSolarSystem: 'MAX_PER_STAR_SYSTEM',
    ConstructionTime: 'UPGRADE_TIME',
    MaxPerSector: 'MAX_PER_SECTOR',
    ResearchSlots: 'RESEARCH_SLOTS',
    BuyInPairs: 'BUY_IN_PAIRS',
    TotalShipmentCRPerDay: 'TID_PLANET_UPG_SHIPMENT_YIELD',
    RSPublicLateJoin_MinStarLevel: 'RS_PUBLIC_LATE_JOIN_MIN_STAR_LEVEL',
    NumPlayersWithAccess: 'NUM_PLAYERS_WITH_ACCESS',
    NumMaxPlanets: 'NUM_MAX_PLANETS',
    MaxInfluenceLoss: 'MAX_INFLUENCE_LOSS',
    MaxInfluenceGain: 'MAX_INFLUENCE_GAIN',
    InfluenceAwardThreshold: 'INFLUENCE_AWARD_THRESHOLD',
    SlotsUsed: 'SLOTS_USED',
    SalvageCRReward: 'SALVAGE_CR_REWARD',
    XP: 'XP_AWARD',
    XPReward: 'XP_AWARD',
    MinRSLevel: 'MIN_RS_LEVEL',
    WSMaxBSPerPlayer: 'WS_MAX_BS_PER_PLAYER',
    WSMaxOtherPerPlayer: 'WS_MAX_OTHER_PER_PLAYER',
    RedStarDisplayLevel: 'LVL',
    WSBucketSizes: 'MODE',
    MaxWaypoints: 'MAX_WAYPOINTS',
    StarSectorNavpoints: 'STAR_SECTOR_NAVPOINTS',
    BlueStar_CRRewardWinLimitPeriod: 'BLS_CR_REWARD_WIN_LIMIT_PERIOD',
    BlueStar_MaxHydroPerDayPct: 'BLS_MAX_HYDRO_PER_DAY_PCT',
    BlueStar_RankedMatchesPerDay: 'BLS_RANKED_MATCHES_PER_DAY',
    BlueStar_SPFactor: 'BLS_SP_FACTOR',
    BlueStar_HydroPctPerPos: 'BLS_HYDRO_REWARD',
    BlueStar_CreditPctPerPos: 'BLS_CREDIT_REWARD',
    ConceptImage: 'PORTRAIT',
    AIUpdateIntervalSeconds: 'AI_UPDATE_INTERVAL',
    ReqBank: 'REQ_BANK',
    DronesIgnoreBarrier: 'DRONES_IGNORE_BARRIER',
    RushCostModifier: 'RUSH_COST_MODIFIER',
    MinScannerLevel: 'CLUSTER',
    PvP: 'PVP',
    PvE: 'PVE',
    ApplyModuleOnDockedObjectDestroy: 'APPLY_MODULE_ON_DOCKED_OBJECT_DESTROY',
    OnDeathHealMaxShips: 'ON_DEATH_HEAL_MAX_SHIPS',
    YSMaxGenesis: 'MAX_PER_STAR_SYSTEM',
    BlastDroneTriggerDist: 'TRIGGER_DIST',
    LinkWeaponRange: 'TRIGGER_DIST',
    NumProjectilesPerSOInSector: 'NUM_PROJECTILES',
    AllowAdditionalBuildingsAtMaxLevel: 'ALLOW_BUILDING_AT_MAX_LEVEL',
    WSLostShipTimeCooldown: 'LOST_SHIP_COOLDOWN',
    WSDisbandCooldown: 'SHIP_DISBAND_COOLDOWN',
    WSJumpBSTimeCooldown: 'COOLDOWN_AFTER_JUMPING_WITH_A_BATTLESHIP',
    WSJumpOtherTimeCooldown: 'COOLDOWN_AFTER_JUMPING_TO_ANOTHER_SHIP_TYPE',
    Moons: 'MOONS',
    TimeToUpgradeParallel: 'UPGRADE_TIME_IN_PARALLEL',
    turret: 'TURRET',
    BlueprintsGranted: 'BLUEPRINTS_GRANTED',

    ...gameCharsLocKeys,
    ...gameSlostLocKeys,
};
